...
class PsiProtocolDO(PsiProtocol):
    ...
    def __one_to_one_exchange_init(self):
        super.status(self,PSI_STATUS.INIT)
    ...
    def psi_setup(self):
        super.status(self,PSI_STATUS.SETUP)
    ...
    def psi_response(self):
        super.status(self,PSI_STATUS.RESPONSE)
...
class PsiStar(PsiProtocolDO):
    ...
    def retrieve_intersection(self):
        id_int = self.__get_object_duet(tag=PSI_TAGS.IDS_INTERSECT)
        id_int_list = list(map(int, list(id_int)))
        return id_int_list
   
    def share_all_values(self, id_int_list):
